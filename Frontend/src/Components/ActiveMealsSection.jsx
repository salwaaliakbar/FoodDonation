...
const ActiveMealsSection = ({ Meals, title }) => {
    return (
        <section className="w-full mx-auto bg-white border-[1px] border-zinc-200 rounded-xl shadow-lg mt-4 mb-4">
            <h2 className="text-xl font-bold text-green-700 p-4 border-b">{title} Meals</h2>
            {Meals.length ? (Meals.map((meal, i) => (
                <MealCard key={i} meal={meal} />
            ))) : (title === 'Active' ?
                <NoMeals message="No active meals found" hint="Start Applying on meals to see them here." /> :
