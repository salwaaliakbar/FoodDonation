...
const MealPostCard = ({ meal, index, setMealPosts }) => {
    ...
    const handleApply = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/applyCampaign?userId=${user._id}`, {
                method: "POST",
                headers: {
    ...
    return (
        <>
            ...
            >
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-sm">{meal.
                            location}</p>
                    </div>
                </div>
                ...
                <div className="flex justify-between items-center text-sm text-gray-700 mt-2">
                    <p>🍴 Meal for {meal.amount} {meal.amount > 1 ? 'persons' : 'person'} </p>
                    <p>Status: {meal.status}</p>
                </div>

                <div className="flex justify-between items-center text-sm text-gray-700 mt-2">
                    <p>⏰ {new Date(meal.createdAt).toLocaleString('en-PK')}</p>
                    <p>👥 {meal.applied.length} Applied {applied && <span className="text-green-600 ml-2">✔️</span>}</p>
                </div>
            ...
            {showModal && <MealApplyModal selectedPeople={selectedPeople} setSelectedPeople={setSelectedPeople} personCount={meal.amount} submitStatus={submitStatus} setShowModal={setShowModal} handleApply={handleApply} />
            }
        </>
    );
};
